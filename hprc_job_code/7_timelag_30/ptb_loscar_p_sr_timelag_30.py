...
def init_start(params):
    ...
    if len(erosion_factor) == 3:
        ...
        fscale_erosion =interpolate.interp1d([-251.98e6, -251.966e6, -251.954e6, -251.94e6, -251.92e6, -251.899e6], [erosion_factor[0], erosion_factor[1], erosion_factor[2], erosion_factor[3], erosion_factor[4],1], bounds_error = False, kind = 'zero', fill_value = 1)
    ...
    if RUN_TYPE == 0 or RUN_TYPE == 3:
        ...
        if len(cinp) == 3:
            ...
            fcinp = interpolate.interp1d([-251.98e6, -251.966e6,  -251.954e6, -251.94e6, -251.92e6, -251.899e6], [cinp[0], cinp[1], cinp[2], cinp[3], cinp[4],0], bounds_error = False, kind = 'zero', fill_value = 0)
        elif len(cinp) == 10:
...
def model_run():
    ...
    if RUN_TYPE == 1 or RUN_TYPE == 0:
        ...
        # t_eval = np.arange(-252e6, -251.899e6, 2e3)
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls],  method = ODE_SOLVER)

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls], method = ODE_SOLVER, first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], method = ODE_SOLVER, first_step = 1e2)
