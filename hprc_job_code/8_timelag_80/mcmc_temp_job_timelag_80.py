...
from ptb_loscar_p_sr_timelag_80 import parameters, init_start, model_run
...
temp_loess = pd.read_excel('for_multi_inversion.xlsx', sheet_name = 'T_loess_80').to_numpy()
...
d13c_loess = pd.read_excel('for_multi_inversion.xlsx', sheet_name = 'd13c_loess_80').to_numpy()

sclim = 4.5

exp_name = f'd13c_sens_test_timelag_80'
...
def log_probability(theta, exp_name = 'test', switch = 1):
    ...
    if not np.isfinite(lp):
        ...
        try:
            ...
            n = int((len(theta)-2)/2)
            params = parameters(d13cin = 3, d13cvc = -2.7, svresults = 1, printflag = 0, pcycle = 2, LOADFLAG = 1, svstart = 0, cinpflag = 1, cinp = 6e3, tcspan = 6e4,  dccinp = -25, nsi = 0.4)
            ...
            fsr = interpolate.interp1d(sr_model[:,0], sr_model[:,1],  bounds_error = False,  fill_value = 'extrapolate')
            ...
            pr_sr = np.sum(norm.logpdf(fsr(sr_proxy[:,0]), sr_proxy[:,1], sr_proxy[:,2]))


            pr_d13c = np.sum(norm.logpdf(fd13c(d13c_loess[:,0]), d13c_loess[:,1], d13c_loess[:,2]))

            if switch == 1:
                pr_temp = np.sum(norm.logpdf(ftemp_loess(temp_loess[:,0]), temp_loess[:,1], temp_loess[:,2]))

                sum_diff = pr_sr + pr_temp + pr_d13c

            elif switch == 2:
                pr_pco2 = np.sum(norm.logpdf(fpco2(pco2_proxy[:,0]), pco2_proxy[:,1], pco2_proxy[:,2]))
                sum_diff = pr_sr + pr_pco2 + pr_d13c
...
def log_prior(theta):
    ...
    if (cinp_over_time<=0.8).all() and (cinp_over_time>=0.01).all() and (erosion_over_time<=5).all() and (erosion_over_time>=0.6).all() and (d13c_over_time>=-40).all() and (d13c_over_time<=-5).all() :
        return 0.0
    return -np.inf
...
if __name__ == '__main__':
    ...
    theta_init = np.array([  0.15714361,   0.12167933,    0.3869759 ,  0.57921896,   0.52326151,   2.34762547,    3.55339855,   1.86813788,  0.82641922,   0.78367564, -10, -15])
    ...
    pos = np.array(theta_init) +  1e-4 * np.random.randn(n_walker, ndim)
    nwalkers, ndim = pos.shape
    moves = [
    (StretchMove(a=2.0), 0.6),
    (DEMove(),            0.3),
    (DESnookerMove(),     0.1),
    ]

    with Pool(processes=n_walker) as pool:
        sampler = emcee.EnsembleSampler(
            nwalkers, ndim, log_probability,
            moves = moves,
            args = ('mcmc_temp_sr', 1), pool = pool)
        pos, lnprob, rstate = sampler.run_mcmc(pos, n_burnin, progress=True)

        # Throw away the burn-in samples before production
        sampler.reset()
