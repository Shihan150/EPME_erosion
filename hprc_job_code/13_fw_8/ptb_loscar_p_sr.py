...
def init_start(params):
    global fw
    fw = interpolate.interp1d([-252e6, -251.94e6,  -251.899e6], [1, 0.8, 0.8], bounds_error = False, fill_value = 1)
    global RUN_TYPE
...
def derivs(t, y, hpls, *flux_flag):
    ...
    fsi = fvc0* (pco2a/pcsi) ** nsi *(fscale_erosion(t) ** 0.8) * fw(t)

    # CaCO3 weathering
    finc = finc0 * (pco2a/pcsi) ** ncc * (fscale_erosion(t) ** 0.86) * fw(t)
