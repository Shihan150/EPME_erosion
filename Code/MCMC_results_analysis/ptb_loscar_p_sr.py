...
def model_run():
    ...
    if RUN_TYPE == 1 or RUN_TYPE == 0:
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls], t_eval = t_eval, method = 'LSODA')

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls], t_eval = t_eval,method = 'LSODA', first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], t_eval = t_eval,method = 'LSODA', first_step = 1e2)



        else:
            first_step = None
            ysol = solve_ivp(derivs, (t0, tfinal), ystart,t_eval = t_eval, args = [ hpls], method = 'BDF')
