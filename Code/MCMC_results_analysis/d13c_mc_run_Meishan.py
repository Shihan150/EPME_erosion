...
def main(i):
    ...
    params = parameters(d13cin = 2, d13cvc = -1.8, svresults = 1, printflag = 0, pcycle = 2, LOADFLAG = 1, svstart = 0, cinpflag = 1, cinp = 6e3, tcspan = 6e4,  dccinp = -25, sclim = 4.5)

    params['nsi'] = float(nsi)
    params['target_file'] = 'ptb_d13c_Meishan.csv'
    ...
    with Pool() as pool:
        ...
        while True:
            try:
                ...
                results.append(np.full([len(t_eval), ], np.nan))
    results = np.array(results)

    np.savetxt(f'd13c_emi_inv_{exp_name}.dat', results)
