...
def parameters(exp_name = 'test', LOADFLAG = 0, RUNTYPE = 1, pcycle = 1, svstart = 0, svresults = 1, printflag = 1,
               t0 = -252.00e6, tfinal = -251.9e6, pref = 425, pcsi = 425, fepl = 0.8, eph = 1.8, rrain = 1e50,
               ...
               svfile = 'ptb_steady.dat', target_file = 'ptb_d13c.csv', toms_low = -0.05, toms_high = 3, dccinp_inv1 = -5, dccinp_inv2 = -5
               ):
    params = {
        'exp_name': exp_name,
        'LOADFLAG': LOADFLAG,
...
def init_start(params):
    ...
    if len(erosion_factor) == 3:
        fscale_erosion =interpolate.interp1d([-252e6, -251.952e6, -251.94e6, -251.9e6], [erosion_factor[0], erosion_factor[1], erosion_factor[2], 1], bounds_error = False, kind = 'zero', fill_value = 1)
    elif len(erosion_factor) == 5:
        fscale_erosion =interpolate.interp1d([-252e6, -251.976e6, -251.954e6, -251.94e6, -251.92e6, -251.90e6], [erosion_factor[0], erosion_factor[1], erosion_factor[2], erosion_factor[3], erosion_factor[4],1], bounds_error = False, kind = 'zero', fill_value = 1)
        ...
        fscale_erosion =interpolate.interp1d([-252e6, -251.99e6,  -251.98e6, -251.97e6, -251.96e6, -251.95e6, -251.94e6, -251.93e6, -251.92e6, -251.91e6,  -251.90e6], [erosion_factor[0], erosion_factor[1], erosion_factor[2], erosion_factor[3], erosion_factor[4], erosion_factor[5],erosion_factor[6], erosion_factor[7], erosion_factor[8], erosion_factor[9],1], bounds_error = False, kind = 'zero', fill_value = 0)
    ...
    if RUN_TYPE == 0 or RUN_TYPE == 3:
        ...
        if len(cinp) == 3:
            fcinp = interpolate.interp1d([-252e6, -251.952e6, -251.94e6, -251.9e6], [cinp[0], cinp[1], cinp[2], 0], bounds_error = False, kind = 'zero', fill_value = 0)
        elif len(cinp) == 5:
            fcinp = interpolate.interp1d([-252e6, -251.976e6, -251.954e6, -251.94e6, -251.92e6, -251.90e6], [cinp[0], cinp[1], cinp[2], cinp[3], cinp[4],0], bounds_error = False, kind = 'zero', fill_value = 0)
        elif len(cinp) == 6:
            ...
            fcinp = interpolate.interp1d([-252e6, -251.99e6,  -251.98e6, -251.97e6, -251.96e6, -251.95e6, -251.94e6, -251.93e6, -251.92e6, -251.91e6,  -251.90e6], [cinp[0], cinp[1], cinp[2], cinp[3], cinp[4], cinp[5],cinp[6], cinp[7], cinp[8], cinp[9],0], bounds_error = False, kind = 'zero', fill_value = 0)
...
def model_run():
    ...
    if RUN_TYPE == 1 or RUN_TYPE == 0:
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls], method = 'LSODA')

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [hpls], method = 'LSODA', first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], method = 'LSODA', first_step = 1e2)



        else:
            first_step = None
            ysol = solve_ivp(derivs, (t0, tfinal), ystart, args = [ hpls], method = 'BDF')
    ...
    if RUN_TYPE == 2:
        ...
        for i in range(len(t_target)-1):
            ...
            if platform.system() == 'Windows':
                ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, args = [hpls], t_eval = [tems[1]], method = 'LSODA')

                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =solve_ivp(derivs, (tems[0], tems[1]), y0, args = [hpls], t_eval = [tems[1]],  method = 'LSODA', first_step = 5e2)


                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =  solve_ivp(derivs, (tems[0], tems[1]), y0, args = [hpls], t_eval = [tems[1]], method = 'LSODA', first_step = 1e2)



            else:
                first_step = None
                ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, args = [ hpls], t_eval = [tems[1]], method = 'LSODA', first_step = first_step)
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [ hpls], method = 'LSODA')

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol =solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [hpls],   method = 'LSODA', first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [hpls],  method = 'LSODA', first_step = 1e2)



        else:
            first_step = None
            ysol = solve_ivp(derivs, (t_target[0], t_target[-1]), ystart, args = [hpls], t_eval = np.arange(t_target[0], t_target[-1], 2e3), method = 'BDF')
    ...
    if RUN_TYPE == 3:
        ...
        for i in range(len(t_target_d13c)-1):
            ...
            if platform.system() == 'Windows':
                ysol = solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [hpls], t_eval = [tems_d13c[1]], method = 'LSODA')

                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [ hpls], t_eval = [tems_d13c[1]], method = 'LSODA', first_step = 5e2)


                    if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                        ysol =  solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [hpls], t_eval = [tems_d13c[1]], method = 'LSODA', first_step = 1e1)



            else:
                first_step = 1e3
                ysol = solve_ivp(derivs, (tems_d13c[0], tems_d13c[1]), y0, args = [hpls], t_eval = [tems_d13c[1]], method = 'BDF')
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (t_target_d13c[0], t_target_d13c[-1]), ystart, args = [ hpls], t_eval = t_eval, method = 'BDF')

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol =solve_ivp(derivs, (t_target_d13c[0], t_target_d13c[-1]), ystart, args = [ hpls], method = 'BDF', t_eval = t_eval, first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (t_target_d13c[0], t_target_d13c[-1]), ystart, args = [ hpls], method = 'BDF',t_eval = t_eval,  first_step = 1e2)



        else:
            first_step = 1e3
            ysol = solve_ivp(derivs, (t_target_d13c[0], t_target_d13c[-1]), ystart, args = [ hpls], method = 'LSODA',t_eval = t_eval,  first_step = first_step)
...
def cost_function(ems_new, tracer_type, hpls, tems, ems):
    if tracer_type == 'd13c_for_emi':
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [ hpls], method = 'LSODA')

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol =solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [ hpls], method = 'LSODA', first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [ hpls], method = 'LSODA', first_step = 1e2)



        else:
            first_step = None
            ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [ hpls], method = 'LSODA', first_step = first_step)
    ...
    if tracer_type == 'd13c':
        ...
        if platform.system() == 'Windows':
            ysol = solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [hpls], method = 'BDF')

            if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                ysol =solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [hpls], method = 'BDF', first_step = 5e2)


                if np.isnan(ysol['y']).any() or len(ysol['y']) == 0:

                    ysol =  solve_ivp(derivs, (tems[0], tems[1]), y0, t_eval = [tems[1]], args = [hpls], method = 'BDF', first_step = 1e1)
...
def derivs(t, y, hpls):
    ...
    fsi = fvc0* (pco2a/pcsi) ** nsi *(fscale_erosion(t) ** 0.53)
