...
#         pr_d13c = np.average(norm.logpdf(fd13c(d13c_loess[:,0]), d13c_loess[:,1], d13c_loess[:,2]))
        ...
        if switch == 1:
            pr_temp = np.average(norm.logpdf(ftemp_loess(temp_meishan[:,0]), temp_meishan[:,1], temp_meishan[:,2]))
...
if __name__ == '__main__':
    ...
    theta_init = np.array([  0.15714361,  0.1,  0.12167933,    0.3869759 ,  0.57921896,   0.52326151,   2.34762547,    2, 3.55339855,   1.86813788,  0.82641922,   0.78367564])
